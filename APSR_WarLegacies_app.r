...
rm(list = ls())
setwd("~/Desktop/American Politcal Science Review/Shiny/App")
working_dir <- getwd()

## ... Loading packages

packages <- c("shiny","shinyWidgets","shinydashboard","tidyverse","readr","bslib","metafor","shinyMatrix","showtext")
...
# 1. List all files with pattern "meta.*.csv"

files <- list.files(path = file.path(working_dir,"Datasets", "Datasets"), 
                    pattern = "^meta\\..*\\.csv$", 
                    full.names = TRUE)

# 2. Read, extract outcome + model_type, assign sample_type and combine
...
metadata <- map_dfr(files, function(f) {
  ...
  model_type <- ifelse(length(parts) > 1, parts[2], "Full")
  
  df %>% mutate(outcome = outcome,
                model_type = case_when(model_type == "biv" ~ "Bivariate",
                                       model_type == "random" ~ "Quasi-Experimental",
                                       TRUE ~ model_type))}
  )


metadata$sample_type <- "All"


# 3. Assigning suitable data types

metadata <- readr::type_convert(metadata)

# 4. Assigning design type

metadata$design <- NA
metadata$design <- ifelse(metadata$did_design == "N" & metadata$iv_design == "N" & metadata$rd_design == "N" & metadata$uesd_design == "N" & metadata$random_design == "N", "observational",
                          ifelse(metadata$did_design == "Y", "did_design", 
                                 ifelse(metadata$iv_design == "Y", "iv_design", 
                                        ifelse(metadata$rd_design == "Y", "rd_design", 
                                               ifelse(metadata$uesd_design == "Y", "uesd_design", 
                                                      ifelse(metadata$random_design == "Y", "random_design", metadata$design))))))

metadata <- metadata %>% select(-c(did_design, rd_design, iv_design, uesd_design, random_design))

metadata <- metadata %>% select(authoryear, yi, vi, coef, se, n, outcome, model_type)

## 5. Read master spread sheet

reports <- read_csv(file = file.path(working_dir, "Datasets", "master_spreadsheet.csv"))

reports <- reports %>%
  extract(
    StudyAuthors,
    into = c("authors", "year"),
    regex = "^(.*)\\s+\\((\\d{4}[a-zA-Z]?)\\)$")%>% rename(Study.ID = dell,
                                                           Authors = authors,
                                                           Year = year,
                                                           Publication = Publisher,
                                                           Conflict = ConflictStudy,
                                                           Publication = Publisher,
                                                           Country = StudyCountry,
                                                           Manuscript.Title = StudyTitle)
